...
paperweight {
    upstreams.register("luminol") {
        ...
        patchFile {
            ...
            outputFile = file("atdeprecated-server/build.gradle.kts")
            patchFile = file("atdeprecated-server/build.gradle.kts.patch")
        }
        patchFile {
            path = "luminol-api/build.gradle.kts"
            outputFile = file("atdeprecated-api/build.gradle.kts")
            patchFile = file("atdeprecated-api/build.gradle.kts.patch")
        }
        patchRepo("paperApi") {
            upstreamPath = "paper-api"
            patchesDir = file("atdeprecated-api/paper-patches")
            outputDir = file("paper-api")
        }
        patchRepo("foliaApi") {
            upstreamPath = "folia-api"
            patchesDir = file("atdeprecated-api/folia-patches")
            outputDir = file("folia-api")
        }
        patchDir("luminolApi") {
            upstreamPath = "luminol-api"
            excludes = listOf("build.gradle.kts", "build.gradle.kts.patch", "paper-patches")
            patchesDir = file("atdeprecated-api/luminol-patches")
